  ...
  Future<void> _enviarReporte() async {
    ...
    if (!aceptaInfo) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text(' Debes aceptar enviar los registros de error.'), backgroundColor: Colors.orange),
      );
      return;
    }
    if (detallesController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text(' Por favor, describe qué hacías cuando se cerró.'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      await FirebaseFirestore.instance.collection('tickets_soporte').add({
        ...
        'categoriaProblema': 'App se cierra sola (Crash)', 
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: const [
                  ...
                  SizedBox(height: 5),
                  Text('Este problema puede ocurrir por falta de memoria en el dispositivo o por conflictos con otras aplicaciones.'),
                ],
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('PASO 1: Diagnóstico', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.deepPurple)),
                  const SizedBox(height: 8),
                  const Text('Para arreglar el fallo, necesitamos ver el reporte de error.'),
                  const SizedBox(height: 8),
                  CheckboxListTile(
                    contentPadding: EdgeInsets.zero,
                    activeColor: Colors.deepPurple,
                    title: const Text('Acepto enviar registros de error (logs) para diagnóstico (Obligatorio)', style: TextStyle(fontSize: 13)),
                    value: aceptaInfo,
                  ...
                  const Divider(),
                  const SizedBox(height: 16),
                  const Text('PASO 2: Detalles', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.deepPurple)),
                  const SizedBox(height: 8),
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Ej: Estaba viendo calificaciones y de repente se cerró...',
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton(
                          ...
                          style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14), side: BorderSide(color: Colors.grey.shade400), foregroundColor: Colors.black87),
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(backgroundColor: Colors.deepPurple, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), elevation: 2),
                          child: _isSending ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('ENVIAR'),
                        ),
                      ),
                    ],
                  ),
                ...
                child: Column(
                  children: [
                    ListTile(
                      ...
                      trailing: IconButton(icon: const Icon(Icons.copy), onPressed: () => _copyToClipboard('soporte@colegio.com', 'Correo copiado')),
                    ),
                  ],
                ),
              ),
